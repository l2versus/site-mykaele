// app/api/admin/crm/channels/telegram/send/route.ts — Enviar mensagem via Telegram Bot
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (conversation.channel.type !== 'telegram') {
      return NextResponse.json({ error: 'Esta conversa não é do Telegram' }, { status: 400 })
    }
    ...
    const chatId = conversation.remoteJid.replace('@telegram', '')

    const result = await sendTelegramMessage(creds.botToken, chatId, text)
    ...
    createAuditLog({
      ...
      entityId: message.id,
      details: { conversationId, channel: 'telegram' },
    })
