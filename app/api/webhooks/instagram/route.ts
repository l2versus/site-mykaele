// app/api/webhooks/instagram/route.ts — Webhook do Instagram (Meta Graph API)
// Recebe mensagens DM e notificações do Instagram Business.
...
/**
 * POST — Recebe mensagens e eventos do Instagram
 */
export async function POST(req: NextRequest) {
  try {
    const bodyText = await req.text()

    // Verificar assinatura HMAC se app secret configurado
    ...
    // Meta envia { object: 'instagram', entry: [...] }
    ...
    for (const entry of entries) {
      ...
      for (const msg of entry.messaging) {
        ...
        // Encontrar canal Instagram pelo recipientId (nosso Instagram Page ID)
        ...
        try {
          ...
          // Redis não disponível — mensagem já salva no banco
        }
      }
    }
