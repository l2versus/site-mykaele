...
import { generateReceiptPDF, downloadReceipt } from '@/utils/pdf-receipt'
...
export default function PaymentSuccessPage() {
  ...
  const handleDownloadReceipt = useCallback(async () => {
    if (generatingPdf || !paymentData) return
    ...
    const processPayment = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.payment) {
            setPaymentData({
              id: data.payment.id || preferenceId || 'N/A',
              amount: data.payment.amount || 0,
              description: data.payment.description || 'Créditos Mykaele Home Spa',
              method: data.payment.method || 'CREDIT_CARD',
              date: data.payment.createdAt || new Date().toISOString(),
              clientName: data.payment.userName || 'Cliente',
              clientEmail: data.payment.userEmail || '',
              clientPhone: data.payment.userPhone
            })
          } else {
            // Dados básicos se não vier do servidor
            setPaymentData({
              id: preferenceId || 'N/A',
              amount: 0,
              description: 'Créditos Mykaele Home Spa',
              method: 'CREDIT_CARD',
        ...
        {status === 'success' && (
          <div className="text-center space-y-6 animate-[fadeIn_0.6s_ease-out]">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.06] transition-all disabled:opacity-50"
            >
              {generatingPdf ? (
                <>
                  <div className="w-4 h-4 border-2 border-white/20 border-t-white/60 rounded-full animate-spin" />
                  <span className="text-white/60 text-[11px] font-medium">Gerando...</span>
                </>
              ) : (
                <>
                  <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-white/50">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                    <polyline points="7 10 12 15 17 10" />
                    <line x1="12" y1="15" x2="12" y2="3" />
                  </svg>
                  <span className="text-white/60 text-[11px] font-medium">Baixar Comprovante PDF</span>
                </>
              )}
            </button>
          </div>
