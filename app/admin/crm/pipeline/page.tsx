...
function InfoItem({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="rounded-xl p-2.5 sm:p-3" style={{ background: 'var(--crm-surface)', border: '1px solid var(--crm-border)' }}>
      <p className="text-[9px] sm:text-[10px] uppercase tracking-wider font-medium mb-0.5 sm:mb-1" style={{ color: 'var(--crm-text-muted)' }}>{label}</p>
      <p className="text-[11px] sm:text-xs font-semibold truncate" style={{ color: accent ?? 'var(--crm-text)' }}>{value}</p>
    </div>
  )
}
...
function LeadDrawer({ leadId, stages, onClose, onLeadUpdated }: {
  ...
  return (
    <motion.div className="fixed inset-0 z-40" initial="hidden" animate="visible" exit="exit">
      ...
      <motion.div
        className="absolute right-0 top-0 bottom-0 w-full sm:max-w-lg sm:border-l flex flex-col overflow-hidden"
      ...
      >
        {isLoading ? <DrawerSkeleton /> : lead ? (
          <>
            {/* Header */}
            <div className="px-3.5 py-3 sm:p-5 border-b" style={{ borderColor: 'var(--crm-border)' }}>
              <div className="flex items-center gap-2.5 sm:gap-3.5">
                <LeadAvatar name={lead.name} status={lead.status} size={38} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <h3 className="text-sm sm:text-base font-bold truncate" style={{ color: 'var(--crm-text)' }}>{lead.name}</h3>
                    <span className="text-[9px] font-bold px-1.5 sm:px-2 py-0.5 rounded-md uppercase tracking-wider shrink-0"
                      style={{ background: statusColor + '12', color: statusColor, border: `1px solid ${statusColor}20` }}>
                      {STATUS_LABELS[lead.status] ?? lead.status}
                    </span>
                  </div>
                  <p className="text-[10px] sm:text-[11px] mt-0.5 truncate" style={{ color: 'var(--crm-text-muted)' }}>
              ...
              {/* Quick info bar */}
              <div className="flex items-center flex-wrap gap-2 sm:gap-4 mt-3 pt-3" style={{ borderTop: '1px solid var(--crm-border)' }}>
            ...
            {/* Tabs */}
            <div className="flex px-1.5 sm:px-2 pt-1 border-b" style={{ borderColor: 'var(--crm-border)' }}>
              {TABS.map(tab => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className="flex-1 py-2.5 sm:py-2.5 text-[10px] sm:text-[11px] font-medium transition-all relative flex items-center justify-center gap-1 sm:gap-1.5 rounded-t-lg"
                  style={{
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto px-3.5 py-3 sm:p-5">
              {activeTab === 'details' && (
                <div className="space-y-3 sm:space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {stages.filter(s => s.id !== lead.stageId).map(s => (
                        <button key={s.id} onClick={() => handleMoveStage(s.id)} disabled={movingStage}
                          className="text-[11px] px-3 py-2 sm:py-1.5 rounded-lg font-medium transition-all hover:brightness-125 disabled:opacity-40 active:scale-[0.97]"
                  ...
                  {lead.patientId ? (
                    <Link href={`/admin/clientes?highlight=${lead.patientId}`}
                      className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-xs sm:text-sm font-semibold transition-all hover:brightness-110 active:scale-[0.98]"
                    ...
                    <button onClick={handleConvertToPatient} disabled={converting}
                      className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-xs sm:text-sm font-semibold transition-all hover:brightness-110 disabled:opacity-40 active:scale-[0.98]"
                      style={{ background: 'rgba(46,204,138,0.06)', color: '#2ECC8A', border: '1px solid rgba(46,204,138,0.12)' }}
                    >
                ...
                <div className="space-y-2.5">
                  {messages.length === 0 ? (
                    ...
                    <div key={msg.id} className={`flex ${msg.fromMe ? 'justify-end' : 'justify-start'}`}>
                      <div className="max-w-[85%] sm:max-w-[80%] rounded-2xl px-3 py-2 sm:px-3.5 sm:py-2.5 text-[12px] sm:text-[13px]"
                        style={{
...
export default function PipelinePage() {
  ...
  useEffect(() => {
    setActionsPortal(document.getElementById('crm-page-actions'))
  }, [])
  ...
  return (
    <div>
      ...
      {/* Header — botões renderizados no sticky nav via portal */}
      {actionsPortal && createPortal(
        <div
          className="flex items-center justify-between gap-3 px-3 lg:px-5 py-2"
          style={{ borderTop: '1px solid var(--crm-border)' }}
        >
          <div className="min-w-0">
            <h1 className="text-sm lg:text-base font-bold tracking-tight truncate" style={{ color: 'var(--crm-text)' }}>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              disabled={importing}
              className="flex items-center gap-2 px-2.5 lg:px-3 py-1.5 lg:py-2 rounded-xl text-[11px] lg:text-xs font-medium whitespace-nowrap transition-all hover:brightness-125 active:scale-[0.98] disabled:opacity-50"
            ...
            <button onClick={() => setShowNewLead(true)}
              className="flex items-center gap-2 px-3 lg:px-4 py-1.5 lg:py-2 rounded-xl text-[11px] lg:text-xs font-bold whitespace-nowrap transition-all hover:brightness-110 active:scale-[0.98]"
