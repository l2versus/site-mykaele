                ...
                <div className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-white transition-all" onClick={() => setExpanded(isExpanded ? null : c.id)}>
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); openCreditModal(c) }}
                      className="p-2.5 sm:p-1.5 rounded-md bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 active:scale-[0.95] transition-all touch-manipulation min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center" title="Inserir créditos">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); openEdit(c) }}
                      className="p-2.5 sm:p-1.5 rounded-md bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 active:scale-[0.95] transition-all touch-manipulation min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center" title="Editar cliente">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    {c.phone && (
                      <button onClick={(e) => { e.stopPropagation(); setMsgModal(c); setCustomMsg('') }}
                        className="hidden sm:flex p-1.5 rounded-md bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all items-center justify-center" title="WhatsApp">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); deleteClient(c) }}
                      className="p-2.5 sm:p-1.5 rounded-md bg-red-500/10 text-red-400 hover:bg-red-500/20 active:scale-[0.95] transition-all touch-manipulation min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center" title="Excluir cliente">
                  ...
                  <div className="border-t border-stone-100 px-4 py-3 space-y-3 bg-stone-50/50">
                    ...
                    <div className="flex gap-1.5 flex-wrap pt-1">
                      <button onClick={() => openCreditModal(c)}
                        className="px-3 py-2.5 sm:px-2.5 sm:py-1.5 rounded-md text-[11px] sm:text-[10px] font-medium bg-amber-500/10 text-amber-500 border border-amber-500/15 hover:bg-amber-500/20 active:scale-[0.97] transition-all flex items-center gap-1 touch-manipulation min-h-[40px] sm:min-h-0">
                      ...
                      <button onClick={() => openEvoModal(c)}
                        className="px-3 py-2.5 sm:px-2.5 sm:py-1.5 rounded-md text-[11px] sm:text-[10px] font-medium bg-violet-500/10 text-violet-500 border border-violet-500/15 hover:bg-violet-500/20 active:scale-[0.97] transition-all flex items-center gap-1 touch-manipulation min-h-[40px] sm:min-h-0">
                      ...
                      {c.phone && (
                        <>
                        {/* WhatsApp button visible on mobile (hidden in header row) */}
                        <button onClick={() => { setMsgModal(c); setCustomMsg('') }}
                          className="sm:hidden px-3 py-2.5 rounded-md text-[11px] font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/15 hover:bg-emerald-500/20 active:scale-[0.97] transition-all flex items-center gap-1.5 touch-manipulation min-h-[40px]">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                          WhatsApp
                        </button>
                        {quickMsgs.map((q, i) => (
                          <button key={i} onClick={() => openWhatsApp(c.phone!, q.msg(c))}
                            className="px-3 py-2.5 sm:px-2.5 sm:py-1.5 rounded-md text-[11px] sm:text-[10px] font-medium bg-emerald-500/8 text-emerald-400/70 border border-emerald-500/15 hover:bg-emerald-500/15 active:scale-[0.97] transition-all touch-manipulation min-h-[40px] sm:min-h-0">
                            {q.label}
                          </button>
                        ))}
                        <button onClick={() => { setMsgModal(c); setCustomMsg('') }}
                          className="hidden sm:block px-2.5 py-1.5 rounded-md text-[10px] font-medium bg-stone-50 text-stone-400 border border-stone-200 hover:text-stone-500 transition-all">
